...
export async function POST(req: Request) {
  try {
    ...
    if (!process.env.RESEND_API_KEY) {
      console.log('RESEND_API_KEY missing — showing reset URL for debugging:', resetUrl)
      return NextResponse.json({
        success: true,
        message: 'If an account exists with this email, a password reset link has been sent.'
      })
    ...
    try {
        ...
        html: `
          ...
          <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <div style="background: #f9f9f9; padding: 30px; border-radius: 10px;">
              ...
              <p style="font-size: 12px; color: #666;">Ou copiez ce lien :</p>
      ...
      return NextResponse.json({
        success: true,
        message:
          'If an account exists with this email, a password reset link has been sent. (Email service error)'
      })
    }
